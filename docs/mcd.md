...
erDiagram
    ...
    DEMANDE_REPARATION ||--o{ REPARATION : "engendre"
    UTILISATEUR ||--o{ REPARATION : "cree (mecanicien ou gestionnaire)"
    REPARATION ||--o{ AFFECTATION : "est assignee via"
    UTILISATEUR ||--o{ AFFECTATION : "recoit (mecanicien)"
    UTILISATEUR ||--o{ AFFECTATION : "decide (gestionnaire)"
    REPARATION ||--o{ INTERVENTION : "est traitee par"
    ...
    REPARATION {
        int id PK
        int demande_id FK
        int createur_id FK "mecanicien ou gestionnaire"
        string titre
        text description
        string statut "a_faire / en_cours / terminee / annulee"
    }
    AFFECTATION {
        int id PK
        int reparation_id FK
        int mecanicien_id FK
...
| RG4 | Une **demande de réparation** porte sur un et un seul véhicule (1,1) et peut découler d'au plus un rendez-vous (0,1). Un véhicule fait l'objet de 0 à n demandes — c'est cette relation qui fournit l'**historique des réparations** (#11). |
| RG5 | Une demande suit le cycle de vie : `soumise → diagnostiquee → en_reparation → terminee → facturee`, avec `annulee` possible avant `en_reparation`. Elle passe à `en_reparation` dès la création d'une première réparation (RG7) et à `terminee` quand toutes ses réparations sont terminées. C'est ce statut que consulte le client (#5). |
...
| RG7 | Une **réparation** est créée pour une et une seule demande (1,1), par un et un seul utilisateur de rôle `mecanicien` ou `gestionnaire` (1,1), et uniquement après que la demande a reçu un diagnostic (RG6). Une demande engendre 0 à n réparations. Une réparation suit le cycle de vie : `a_faire → en_cours → terminee`, avec `annulee` possible. |
| RG8 | Une **affectation** lie une réparation (1,1) à un mécanicien (1,1) et est décidée par un gestionnaire (1,1). Une réparation a au plus une affectation `active` à la fois; les affectations annulées sont conservées pour la traçabilité. |
| RG9 | Une **intervention** est effectuée par un et un seul mécanicien (1,1) et se rattache à une et une seule réparation (1,1); une réparation peut compter 0 à n interventions. Seul le mécanicien de l'affectation `active` lie ses interventions à la réparation (contrainte applicative). |
| RG10 | Une intervention consomme 0 à n **pièces** via des lignes de pièce; chaque ligne fige la quantité et le prix unitaire au moment de l'utilisation. La quantité en stock de la pièce est décrémentée d'autant. |
| RG11 | Une **facture** est émise pour une et une seule demande, et une demande a au plus une facture (0,1). Le total = main-d'œuvre + pièces + TPS + TVQ. |
| RG12 | Une facture est réglée par 0 à n **paiements** (paiements partiels permis); elle passe à `payee` quand la somme des paiements atteint le montant total. |
| RG13 | Une **notification** a un et un seul destinataire; un utilisateur reçoit 0 à n notifications. |
| RG14 | Une **évaluation** est rédigée par le client de la demande, au plus une par demande (0,1), et seulement lorsque la demande est `terminee` ou `facturee`. La note est un entier de 1 à 5. |
| RG15 | Seul un utilisateur de rôle `mecanicien` peut être visé par les relations « rédige un diagnostic », « effectue une intervention » et « reçoit une affectation »; seul un `mecanicien` ou un `gestionnaire` crée une réparation; seul un `gestionnaire` décide d'une affectation; seul un `client` possède des véhicules. (Contrainte applicative, validée dans les serializers.) |
...
| role | CharField(20) | TextChoices : `client`, `mecanicien`, `gestionnaire`, `administrateur` | Rôle unique (RG1, RG15) |
...
### 3.4 DemandeReparation (#4, #5, #11 — MG / BI / SLG)
...
### 3.5 Diagnostic (#6 — GAM)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| demande | OneToOneField(DemandeReparation) | requis, PROTECT, `related_name='diagnostic'` | Demande diagnostiquée (RG6) |
| mecanicien | ForeignKey(Utilisateur) | requis, PROTECT, `related_name='diagnostics'` | Auteur (rôle `mecanicien`, RG15) |
...
### 3.7 Reparation (#10 — GAM)

Créée par un mécanicien ou un gestionnaire une fois la demande diagnostiquée; regroupe les travaux à réaliser et sert de point d'attache aux affectations et aux interventions.

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| demande | ForeignKey(DemandeReparation) | requis, PROTECT, `related_name='reparations'` | Demande d'origine, déjà diagnostiquée (RG7) |
| createur | ForeignKey(Utilisateur) | requis, PROTECT, `related_name='reparations_creees'` | Mécanicien ou gestionnaire (RG15) |
| titre | CharField(200) | requis | Résumé des travaux |
| description | TextField | requis | Travaux à réaliser (issus du diagnostic) |
| statut | CharField(20) | TextChoices : `a_faire`, `en_cours`, `terminee`, `annulee`; défaut `a_faire` | Cycle de vie (RG7) |

### 3.8 Affectation (#10 — GAM)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| reparation | ForeignKey(Reparation) | requis, CASCADE, `related_name='affectations'` | Réparation assignée (RG8) |
| mecanicien | ForeignKey(Utilisateur) | requis, PROTECT, `related_name='affectations_recues'` | Mécanicien assigné (RG15) |
| gestionnaire | ForeignKey(Utilisateur) | requis, PROTECT, `related_name='affectations_decidees'` | Décideur (RG15) |
| date_affectation | DateField | `auto_now_add` | Date de la décision |
| statut | CharField(20) | TextChoices : `active`, `annulee`; défaut `active` | Une seule `active` par réparation (RG8) |

### 3.9 Intervention (#7 — SLG) — *existe déjà*

Champs actuels de `backend/interventions/models.py`, avec les évolutions prévues (voir §6).

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| reparation | ForeignKey(Reparation) | requis, PROTECT, `related_name='interventions'` | **Remplace** le champ texte `vehicule` actuel (RG9) |
...
### 3.10 Piece et LignePiece (#15 — GAM)
...
| quantite_stock | PositiveIntegerField | défaut 0 | Stock disponible (RG10) |
...
| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| intervention | ForeignKey(Intervention) | requis, CASCADE, `related_name='lignes_pieces'` | Intervention consommatrice (RG10) |
...
| prix_unitaire_facture | DecimalField(8,2) | requis | Prix figé au moment de l'utilisation (RG10) |

### 3.11 Facture (#12 — BI)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| demande | OneToOneField(DemandeReparation) | requis, PROTECT, `related_name='facture'` | Demande facturée (RG11) |
...
| montant_total | DecimalField(8,2) | requis | Total TTC (RG11) |
| statut | CharField(20) | TextChoices : `emise`, `payee`, `annulee`; défaut `emise` | Passe à `payee` selon RG12 |

### 3.12 Paiement (#13 — MG)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| facture | ForeignKey(Facture) | requis, PROTECT, `related_name='paiements'` | Facture réglée (RG12) |
...
### 3.13 Notification (#16 — SLG)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| destinataire | ForeignKey(Utilisateur) | requis, CASCADE, `related_name='notifications'` | Destinataire (RG13) |
...
### 3.14 Evaluation (#17 — AN)

| Attribut | Type Django | Contraintes | Description |
|---|---|---|---|
| demande | OneToOneField(DemandeReparation) | requis, PROTECT, `related_name='evaluation'` | Demande évaluée (RG14) |
| client | ForeignKey(Utilisateur) | requis, PROTECT, `related_name='evaluations'` | Auteur (client de la demande, RG14) |
...
- **#5 Consulter l'état des réparations** : lecture de `DemandeReparation.statut` (+ diagnostic, réparations et interventions liés).
- **#11 Historique des réparations d'un véhicule** : `Vehicule → demandes → reparations → interventions/factures`, trié par date.
- **#14 Rechercher les dossiers clients** : recherche sur `Utilisateur` + agrégation de ses véhicules, demandes et factures.
- **#18 Rapports et statistiques** : agrégations (revenus par période, charge par mécanicien, pièces les plus utilisées…).
...
4. **Suppressions** : `on_delete=PROTECT` par défaut vers les entités de traçabilité (utilisateurs, véhicules, demandes, réparations, factures, pièces); `CASCADE` uniquement pour les compositions (lignes de pièce, notifications, affectations d'une réparation). Les comptes utilisateurs se **désactivent**, ne se suppriment pas (RG1).
...
7. **Organisation des apps Django** (une app par domaine, pour limiter les conflits de merge) : `comptes` (Utilisateur), `vehicules`, `reparations` (RendezVous, DemandeReparation, Diagnostic, TypeReparation, Reparation, Affectation), `interventions` (existante : Intervention, LignePiece), `facturation` (Facture, Paiement), `inventaire` (Piece), `notifications`, `evaluations`. Ajustable en équipe, mais à décider **avant** de créer les modèles.
...
|---|---|---|---|
| `vehicule = CharField(200)` | #4 (demandes) et la création des réparations (#10) n'existaient pas encore | `reparation = ForeignKey(Reparation)` — le véhicule se déduit de `reparation.demande.vehicule` | Dès que #4 et #10 sont fusionnées (migration de données : créer les réparations correspondantes ou vider la table de dev) |
| `pieces_utilisees = TextField` | #15 (inventaire) n'existait pas encore | Lignes `LignePiece` liées à l'intervention | Sprint 3, avec #15 |
...
| #3 | Rendez-vous | BI | RendezVous | Utilisateur, Vehicule |
| #4 | Demandes de réparation | MG | DemandeReparation | Vehicule, RendezVous |
| #5 | État des réparations | BI | — (vue, §4) | DemandeReparation, Diagnostic, Reparation, Intervention |
| #6 | Diagnostics | GAM | Diagnostic | DemandeReparation, TypeReparation, Utilisateur |
| #7 | Interventions mécaniques | SLG | Intervention *(existante)* | Reparation, Utilisateur |
| #8 | Utilisateurs et rôles | AN | — (champ `role` de Utilisateur) | Utilisateur |
| #9 | Types de réparations | MG | TypeReparation | — |
| #10 | Affectation aux mécaniciens | GAM | Reparation, Affectation | DemandeReparation, Diagnostic, Utilisateur |
| #11 | Historique des réparations | SLG | — (vue, §4) | Vehicule, DemandeReparation, Reparation, Intervention, Facture |
| #12 | Factures | BI | Facture | DemandeReparation, Intervention, LignePiece |
| #13 | Paiements | MG | Paiement | Facture |
| #14 | Dossiers clients | AN | — (vue, §4) | Utilisateur, Vehicule, DemandeReparation, Facture |
| #15 | Pièces et inventaire | GAM | Piece, LignePiece | Intervention |
| #16 | Notifications et rappels | SLG | Notification | Utilisateur, RendezVous, DemandeReparation, Facture |
| #17 | Évaluations clients | AN | Evaluation | DemandeReparation, Utilisateur |
| #18 | Rapports et statistiques | MG | — (vue, §4) | toutes |
