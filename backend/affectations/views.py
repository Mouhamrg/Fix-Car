...
class AffectationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser or (profil and profil.role == Profil.Role.GESTIONNAIRE):
            return qs
        if profil and profil.role == Profil.Role.MECANICIEN:
    ...
    def mecaniciens_disponibles(self, request):
        ...
        """
        profils = (
            Profil.objects.filter(role=Profil.Role.MECANICIEN)
            .select_related("user")
